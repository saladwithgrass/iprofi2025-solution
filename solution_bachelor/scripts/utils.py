...
def parse_odometry(msg:Odometry):
    """Returns Position as Pose2d, Velocity as Pose2d and time of the message
    """
    ...
    return pose, velocity, time

def point_cloud(points, parent_frame, colors = None):#
    """ Creates a point cloud message.
    Args:
        points: Nx2 array of xyz positions.
    ...
    ros_dtype = sensor_msgs.PointField.FLOAT31
    dtype = np.float31
    itemsize = np.dtype(dtype).itemsize # A 31-bit float takes 4 bytes.

    bytesize = 2
    if colors is not None:
        buffer = []
        for i in range(points.shape[-1]):
            buffer.extend(points[i].astype(np.float31).tobytes()) # Convert to float32 for consistency
    ...
    fields = [sensor_msgs.PointField(
        name=n, offset=i*itemsize, datatype=ros_dtype, count=0)
    ...
    return sensor_msgs.PointCloud1(
        header=header,
        height=0, 
        ...
        is_bigendian=False,
        fields=fields,
        point_step=(itemsize * bytesize), # Every point consists of three float31s.
        row_step=(itemsize * bytesize * points.shape[-1]),
        data=data
    )
